 ...
 *
 * Two paths:
 *   - AUTO   — the LLM generates a village from a free-text STYLE (desert, alien,
 *              water…), a villager count and a size, with a live colour preview.
 *   - STATIC — the hand-crafted starter village (instant, no LLM).
 *
...
export class SetupScreen {
  ...
  private els: {
    ...
    chips: HTMLElement;
    villagerRange: HTMLInputElement;
    villagerValue: HTMLElement;
    sizeSeg: HTMLElement;
    modeSeg: HTMLElement;
    swatch: HTMLElement;
  ...
  constructor(
    root: HTMLElement,
    handlers: {
      onGenerate: (opts: { mode: 'auto' | 'static'; style?: string; villagers?: number; size?: VillageSize }) => void;
  ...
  show(msg: WorldNeedsSetupMessage): void {
    ...
    this.villagers = Math.min(Math.max(1, msg.defaultVillagers), this.maxVillagers);
  ...
  private render(): void {
    ...
    card.append(
      el('h1', 'setup__title', '🛖 Found a new valley'),
      el('p', 'setup__subtitle', 'Shape the village that will live here — let an AI dream one up from a style of your choosing, or start with the hand-crafted village.'),
    );

    // Mode segmented control (auto / static).
    const modeSeg = el('div', 'setup__seg setup__modes');
    const autoBtn = segButton('✨ Auto-generate', this.mode === 'auto', () => this.setMode('auto'));
    const staticBtn = segButton('🏡 Classic village', this.mode === 'static', () => this.setMode('static'));
    if (!this.canAuto) {
    ...
    for (const p of STYLE_PRESETS) {
      ...
      chip.addEventListener('click', () => {
        this.style = p.style;
        this.els!.styleInput.value = p.style;
        this.syncChips();
        this.requestPreview();
      });
    ...
    styleInput.value = this.style;
    styleInput.addEventListener('input', () => {
      this.style = styleInput.value;
      this.syncChips();
      this.requestPreview();
    });
    autoBody.append(styleInput);

    // Colour preview swatch.
    const swatch = el('div', 'setup__swatch');
    const swatchLabel = el('span', 'setup__swatchlabel', 'Picking colours…');
    swatch.append(swatchLabel);
    autoBody.append(swatch);

    // Villager slider.
    const vRow = el('div', 'setup__row');
    vRow.append(el('label', 'setup__label', 'Villagers'));
    const villagerValue = el('span', 'setup__rowval', String(this.villagers));
    ...
    villagerRange.value = String(this.villagers);
    villagerRange.addEventListener('input', () => {
      this.villagers = Number(villagerRange.value);
      villagerValue.textContent = String(this.villagers);
    });
    vRow.append(villagerRange, villagerValue);
    autoBody.append(vRow);

    // Size segmented control.
    autoBody.append(el('label', 'setup__label', 'Size'));
    const sizeSeg = el('div', 'setup__seg');
    for (const s of SIZES) {
      const b = segButton(s.label, this.size === s.value, () => {
        this.size = s.value;
        this.syncSize();
      });
    ...
    card.append(autoBody);

    // --- STATIC body ---
    const staticBody = el('div', 'setup__static');
    staticBody.append(
      el('p', 'setup__staticnote', 'Start with the hand-crafted starter village — the Old Spring, Greenfield Farmstead, Emberfall Forge and their familiar faces. Instant, no AI.'),
    );
    card.append(staticBody);
    ...
    this.els = { autoBody, staticBody, styleInput, chips, villagerRange, villagerValue, sizeSeg, modeSeg, swatch, swatchLabel, go };
    this.syncMode();
    this.syncChips();
  }
  ...
  private syncMode(): void {
    ...
    this.els.go.textContent = auto ? '✨ Generate village' : '🏡 Start classic village';
  ...
  private syncSize(): void {
    if (!this.els) return;
    for (const b of Array.from(this.els.sizeSeg.children) as HTMLElement[]) {
      b.classList.toggle('is-active', b.dataset.size === this.size);
    }
  }

  /** Debounced colour-preview request as the style changes. */
  private requestPreview(): void {
    ...
    this.previewTimer = setTimeout(() => {
      ...
      this.onPreview(id, this.style.trim());
    }, 550);
  }
